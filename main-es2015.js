...
const redirectLoggedInToHome = () => Object(_angular_fire_auth_guard__WEBPACK_IMPORTED_MODULE_6__["redirectLoggedInTo"])(['system/home']);
const routes = [
...
let HeaderComponent = class HeaderComponent {
    constructor(authService, router, route, userService, afAuth) {
...
let LoginComponent = class LoginComponent {
    constructor(authService, userServise, title, router, route) {
        this.authService = authService;
    ...
    onSubmit() {
        ...
        this.userServise.signInByEmail(email, password).then((result) => {
            this.userServise.getUserData()
                ...
                if (userData) {
                    ...
                    const newUserData = new _shared_module_userData_model__WEBPACK_IMPORTED_MODULE_8__["UserData"](result.user.email, 'user', uId);
    ...
    onRegGoogle() {
        this.userServise.createNewUserByGoogle().then((result) => {
            this.userServise.getUserData()
                .subscribe((userData) => {
                if (userData) {
                    this.login(userData);
    ...
    onRegFacebook() {
        this.userServise.createNewUserByFacebook().then((result) => {
            this.userServise.getUserData()
                .subscribe((userData) => {
                if (userData) {
                    this.login(userData);
...
let RegistrationComponent = class RegistrationComponent {
    constructor(usersServise, router, title, authService) {
    ...
    onSubmit() {
        ...
        const user = new _shared_module_user_model__WEBPACK_IMPORTED_MODULE_3__["User"](email, password);
        ...
        this.usersServise.createNewUserByEmail(user)
            ...
            const userData = new _shared_module_userData_model__WEBPACK_IMPORTED_MODULE_8__["UserData"](result.user.email, 'user', uId);
    ...
    onRegGoogle() {
        this.usersServise.createNewUserByGoogle().then((result) => {
    ...
    onRegFacebook() {
        this.usersServise.createNewUserByFacebook().then((result) => {
...
let AuthResovler = class AuthResovler {
    ...
    resolve(route, state) {
        ...
        if (id) {
            return this.userService.getUserDataByKey(id).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["delay"])(2000), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["take"])(1), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])((data) => data));
        }
        else {
...
let AuthService = class AuthService {
    constructor(afAuth, userService) {
        this.afAuth = afAuth;
        this.userService = userService;
        this.isLogged = false;
...
let UserService = class UserService {
    ...
    createNewUserByEmail(user) {
        console.log('СreateNewUserByEmail user.email=', user.email);
        return this.afAuth.auth.createUserWithEmailAndPassword(user.email, user.password);
    }
    createNewUserByGoogle() {
        return this.afAuth.auth.signInWithPopup(new firebase__WEBPACK_IMPORTED_MODULE_2__["auth"].GoogleAuthProvider()).then((result) => {
...
let CreateQuestionComponent = class CreateQuestionComponent {
    constructor(questionService, db, userService, router) {
    ...
    onSubmit() {
        ...
        const questionNew = new _shared_models_questionNew_model__WEBPACK_IMPORTED_MODULE_5__["QuestionNew"](this.userService.getUserId(), ((new Date()) + ''), 'notApproved', arrTags, text, title);
...
let EddingQuestionComponent = class EddingQuestionComponent {
    constructor(questionService, db, userService, route, router, location) {
        this.questionService = questionService;
        this.db = db;
        this.userService = userService;
        this.route = route;
...
EddingQuestionComponent.ctorParameters = () => [
...
let HeaderComponent = class HeaderComponent {
    constructor(authService, route, router, userService, afAuth, activatedRoute) {
        this.authService = authService;
        this.route = route;
        this.router = router;
        this.userService = userService;
...
let HomeFilterComponent = class HomeFilterComponent {
    constructor() {
...
let QuestionComponent = class QuestionComponent {
    constructor(route, questionService, userService, router, authService) {
        this.route = route;
        this.questionService = questionService;
        this.userService = userService;
        this.router = router;
...
class CommentNew {
    constructor(text, authorID, author, status, date) {
        this.text = text;
...
class QuestionNew {
    constructor(author, date, status, tags, text, title) {
        this.author = author;
        this.date = date;
        this.status = status;
        this.tags = tags;
...
let FilterByDatePipe = class FilterByDatePipe {
    transform(value, isAscending = true) {
        return value.sort((question1, question2) => {
...
let QuestionService = class QuestionService {
    ...
    updateQuestionParam(key, param, value) {
        const obj = {};
        obj[param] = value;
        this.db.object('/questions/' + key).update(obj);
    }
    getAllQuestions() {
        return this.questions.snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])((changes) => {
    ...
    getComments(k) {
        return this.db.list('comments/' + k).snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])((changes) => {
            return changes.map((c) => {
                return Object.assign({}, c.payload.val(), { key: c.payload.key });
            });
        }));
    }
    changeCommentStatus(questionKey, commentKey, value) {
...
let SystemComponent = class SystemComponent {
    constructor(authService, usersServise, title, router, activatedRoute) {
        this.authService = authService;
        this.usersServise = usersServise;
        this.title = title;
        this.router = router;
